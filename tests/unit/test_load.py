...
def test_carregar_sem_permissao(tmpdir, dados_exemplo):
    ...
    assert "Permission denied" in str(exc_info.value), "Deveria falhar com erro de permissão"
...
def test_carregar_sucesso(pasta_saida_mock, dados_exemplo):
    ...
    assert os.path.exists(caminho_arquivo_saida), "O arquivo Excel de saída não foi criado."
